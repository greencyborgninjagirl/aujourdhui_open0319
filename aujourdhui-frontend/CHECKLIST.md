...
- 根目录没有 `vite.config.js` 属正常，由 `@dcloudio/uni-h5` 提供构建。
...
## 四、根目录（aujourd'hui）与前端关系

- 根目录是**后端 + 知识库**：`api.py`、`app.py`、`draw_logic.py`、`requirements.txt` 等，与前端是否白屏无直接关系。
- 前端是**独立工程**，所有运行与编译都在 `aujourdhui-frontend` 内完成；HBuilderX 只需打开并运行该文件夹。
